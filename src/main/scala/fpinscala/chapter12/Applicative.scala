...
trait Applicative[F[_]] extends Functor[F] {
  ...
  def relicateM[A](n: Int, fa: F[A]): F[List[A]] =
...
trait Traverse[F[_]] extends Functor[F] with Foldable[F] {
  ...
  def fuse[M[_], N[_], A, B](fa: F[A])(f: A => M[B], g: A => N[B])(implicit M: Applicative[M],
...
object Traverse {
  ...
  val treeTraverse: Traverse[Tree] = new Traverse[Tree] {
    override def traverse[M[_], A, B](ta: Tree[A])(f: A => M[B])(implicit G: Applicative[M]): M[Tree[B]] =
